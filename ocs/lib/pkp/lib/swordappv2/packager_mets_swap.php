...
class PackagerMetsSwap {
    ...
    //UNLP fix: the title of the journal
    public $sac_jtitle;
    
    //UNLP fix: the issue number
    public $sac_issuenumber;
    ...
    // The abstracts of the item  
    public $sac_abstracts;
    ...
    function __construct($sac_rootin, $sac_dirin, $sac_rootout, $sac_fileout) {
        ...
        //fix: multiple titles, identifiers and abstracts
        $this->sac_identifiers = array();
        $this->sac_titles = array();
        $this->sac_abstracts = array();
    ...
    function setJtitle($sac_thejtitle) {
        $this->sac_jtitle = $this->clean($sac_thejtitle);
    }
    
    function setIssuenumber($sac_theissuenumber) {
        $this->sac_issuenumber = $this->clean($sac_theissuenumber);
    }
    
    function setReallink($sac_thereallink) {
        $this->sac_reallink = $this->clean($sac_thereallink);
    }
    ...
    function addTitle($sac_thetitle,$sac_thelocale) {
        $this->sac_titles[$sac_thelocale]= $this->clean($sac_thetitle);
    }
    
    function addAbstract($sac_theabstract,$sac_thelocale) {
        $this->sac_abstracts[$sac_thelocale]=$this->clean($sac_theabstract);
    }
    ...
    function addIdentifier($sac_theidentifier){
    	$this->sac_identifiers[]=$this->clean($sac_theidentifier);
    }
    ...
    function writeDmdSec($fh) {
        ...
        //UNLP fix: issue and journal title, first put the issue
        
        if (isset($this->sac_issuenumber) && (isset($this->sac_jtitle))){
	        $string=$this->sac_issuenumber.'*'.$this->sac_jtitle;
	        $this->statement($fh,
...
//        }-->multiple title & abst fix
		foreach ($this->sac_titles as $sac_locale => $sac_title) {
            $this->statement($fh, 
                             "http://purl.org/dc/elements/1.1/title", 
                     $this->valueStringLang($sac_title,$sac_locale));
                     //$this->valueString($sac_title));    
        }
        
        foreach ($this->sac_abstracts as $sac_locale => $sac_abstract) {
            $this->statement($fh, 
                             "http://purl.org/dc/terms/abstract", 
                     $this->valueStringLang($sac_abstract,$sac_locale));
                     //$this->valueString($sac_abstract));    
        }
...
//        }
		...
		//UNLP fix: support several identifiers
        ...
        $this->statementVesURI($fh, 
                       ...
                       $this->valueString($this->sac_language));//fix: solve language    
    ...
    function valueStringSesURI($sesURI, $value) {
      ...
      /**UNLP Lang Adater to epdcx:ValueString with lang**/
    function valueStringLang($value,$lang){
	    return "<epdcx:valueString lang=\"" . $lang . "\">" .
		$value . 
		"</epdcx:valueString>\n";
    }
