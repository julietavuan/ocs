...
class SwordImportExportPlugin extends ImportExportPlugin {
	...
	function register($category, $path) {
		//TODO arreglar este error de nombress
		import('UNLPchange.clases.sword1.OJSSwordDeposit_UNLP');
	...
	function deposit($url, $username, $password, $articleId, $depositEditorial, $depositGalleys) {
		...
		$deposit = new OJSSwordDeposit_UNLP($publishedArticle);
		$deposit->setMetadata();
	...
	function display(&$args, $request) {
		...
		switch (array_shift($args)) {
			...
			default:
				...
				$issue =& Request::getUserVar('issue');
				
				if ($issue) {
					$articleIds	= array_map( function($a) { return $a->getId(); } , $publishedArticleDao->getPublishedArticles($issue));
					$templateMgr->assign('issue', $issue);
				}
				else 
					$articleIds = $publishedArticleDao->getPublishedArticleIdsAlphabetizedByJournal($journal->getId(), false);
				
				//$issuesIds = $issueDao->getPublishedIssuesByNumber($journal->getId()); 
				
				$allIssuesIterator = $issueDao->getPublishedIssues($journal->getId());
				$issueMap = array();
				while ($the_issue =& $allIssuesIterator->next()) {
					$issueMap[$the_issue->getId()] = $the_issue->getIssueIdentification();
					unset($the_issue);
				}
				...
				if (!empty($swordUrl)) {
					//UNLPFIX: keep using the sword1 client
					$client = new SWORDAPPClient_sword1();
					//endfix
